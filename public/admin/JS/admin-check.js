// ===============================
// admin-check.js (ปรับปรุงใหม่) - ใช้ Permission System จาก server.js
// ===============================
...
async function initializeApp() {
    try {
        ...
        // โหลดข้อมูลผู้ใช้และ permissions
...
async function loadUserProfile() {
    try {
        ...
        if (sideImg) {
            ...
            sideImg.src = src + (src.includes('?') ? '&' : '?') + 't=' + Date.now(); // กันแคช
        }
...
function updateUIBasedOnPermissions() {
    // แสดงข้อมูลขอบเขตการดูแล
    const headerEl = document.querySelector('.header');
    if (headerEl && userPermissions) {
        const scopeInfo = document.createElement('div');
        scopeInfo.className = 'permission-info';
        scopeInfo.innerHTML = `
            <div style="background: #e3f2fd; padding: 10px; border-radius: 8px; margin-top: 15px; border-left: 4px solid #3498db;">
                <strong>ขอบเขตการดูแล:</strong> ${userPermissions.role_name}
                <br>
                <small style="color: #666;">
                    ${userPermissions.can_manage.includes('all') ?
                'ดูแลข้อมูลการเข้างานของพนักงานทุกคน' :
                `ดูแลเฉพาะพนักงานในความรับผิดชอบ`}
                </small>
            </div>
        `;
        headerEl.appendChild(scopeInfo);
    }

    // ซ่อน/แสดงฟีเจอร์ตาม permissions
    const hasManageAttendance = userPermissions.permissions.includes('manage_attendance');

    // CSV Import section
    const importSection = document.querySelector('.import-section');
    if (importSection) {
        if (hasManageAttendance) {
...
// โหลดข้อมูลการเข้างาน - ใช้ API ใหม่
async function loadAttendanceData() {
    ...
    try {
        ...
        if (!responseData.success) {
            throw new Error(responseData.message || 'Failed to load attendance data');
        }

        const allData = responseData.data || [];

        // แปลงข้อมูลให้ตรงกับรูปแบบที่ต้องการ
        allAttendance = allData.map(record => ({
            id: record.id,
        ...
        const filterInfo = responseData.filter_applied || 'Unknown';
        showNotification(`โหลดข้อมูลสำเร็จ ${allAttendance.length} รายการ (${filterInfo})`, 'success');
...
// ใช้ตัวกรอง - Client-side filtering (เหมือน owner-check.js)
...
function displayPreview(data) {
    ...
    importPreview.insertBefore(summaryDiv, importPreview.querySelector('table'));

    importPreview.style.display = 'block';
}

// นำเข้าข้อมูล CSV - ใช้ API ใหม่
async function importCsvData() {
    ...
    // ตรวจสอบสิทธิ์
    if (!userPermissions.permissions.includes('manage_attendance')) {
        showError('คุณไม่มีสิทธิ์นำเข้าข้อมูลการเข้างาน');
        return;
    }
    ...
    try {
        ...
        const response = await fetch('/api/hr/attendance/bulk-record', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            credentials: 'include', // ส่ง session cookie ไปด้วย
            body: JSON.stringify({ records: csvData }) // แก้เป็น records ตามที่ API คาดหวัง
        });
...
function showLoading(show, message = 'กำลังโหลดข้อมูล...') {
    ...
    if (show) {
        if (loadingEl) {
            ...
            const loadingMessage = loadingEl.querySelector('span');
